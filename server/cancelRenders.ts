 ...
 * running — and the RunPod lanes bill by RUNNING time, not by output. Measured: an untracked
 ...
 * whether it is stopped or not; only the self-hosted lanes (InfiniteTalk and LTX, each on its
 * own RunPod endpoint) leak. A 69Labs b-roll task is likewise billed per output and its ids
 * are not RunPod jobs, so `renderProvider` decides what is touched rather than the mere
...
import { LtxLipsyncAdapter } from "./providers/ltx-lipsync";
...
/**
 * Tell RunPod to stop every host render `job` still has in flight, on whichever of the two
 * self-hosted lanes issued it. Returns how many were asked to stop.
 *
 * Best-effort by construction: both adapters' `cancelJob` never throw, so failing to cancel
...
export async function cancelJobProviderRenders(
  ...
  let n = 0;
  const infinitetalk = ENV.runpodInfinitetalkEndpoint ? idsFor("runpod") : [];
  if (infinitetalk.length) {
    ...
    for (const id of infinitetalk) await runpod.cancelJob(id);
    n += infinitetalk.length;
  }
  const ltxIds = ENV.runpodLtxEndpoint ? idsFor("ltx") : [];
  if (ltxIds.length) {
    const ltx = new LtxLipsyncAdapter(ENV.runpodLtxEndpoint, ENV.runPodApiKey);
    for (const id of ltxIds) await ltx.cancelJob(id);
    n += ltxIds.length;
  }
  if (n)
    console.log(
      `[Longform ${job.id ?? "?"}] ${reason}: told RunPod to stop ${n} in-flight host render(s)`
    );
  return n;
}
