 ...
 * generated to match that channel's voice and niche and stored in this app's R2 bucket under
 * `music/beds/<set>/`. Assembly lays these under the master narration at MUSIC_BED_DUCK_DB on
 * the `planMusicSchedule` music/silence pattern.
 *
 ...
 *
 * Set names are `channelKey` values AND R2 object-key prefixes — the two must stay in sync, so
 * renaming one means renaming the `music/beds/<set>/` prefix in the bucket first. A channel with
 * no set of its own falls back to DEFAULT_MUSIC_CHANNEL and logs one warn line per job.
 */
export const CHANNEL_MUSIC_BEDS: Record<string, string[]> = {
  // Fingerpicked steel-string; plain trusted-neighbor tone.
  garrys_lawn: [
    "music/beds/garrys_lawn/bed-01.mp3",
    "music/beds/garrys_lawn/bed-02.mp3",
  ],
  // Curious question-and-answer figure; never suspenseful.
  petes_lawn_secrets: [
    "music/beds/petes_lawn_secrets/bed-01.mp3",
    "music/beds/petes_lawn_secrets/bed-02.mp3",
  ],
  // Warm guitar with mandolin above it.
  david_wright: [
    "music/beds/david_wright/bed-01.mp3",
    "music/beds/david_wright/bed-02.mp3",
  ],
  // Guitar + hammered dulcimer; plain, nothing sacred.
  david_yoder: [
    "music/beds/david_yoder/bed-01.mp3",
    "music/beds/david_yoder/bed-02.mp3",
  ],
  // Autoharp rather than dulcimer, so it never sounds like the same cue as david_yoder.
  elias_miller: [
    "music/beds/elias_miller/bed-01.mp3",
    "music/beds/elias_miller/bed-02.mp3",
  ],
  // Appalachian mountain idiom in a MAJOR key: modal reads mournful.
  roy_mullins: [
    "music/beds/roy_mullins/bed-01.mp3",
    "music/beds/roy_mullins/bed-02.mp3",
  ],
  // Western-swing lilt with pedal steel, no kit.
  travis_tiny_kolbe: [
    "music/beds/travis_tiny_kolbe/bed-01.mp3",
    "music/beds/travis_tiny_kolbe/bed-02.mp3",
  ],
  // Sustained guitar + wooden flute; no percussion, no pastiche.
  wes_kingfisher: [
    "music/beds/wes_kingfisher/bed-01.mp3",
    "music/beds/wes_kingfisher/bed-02.mp3",
  ],
  // The brightest set; lift comes from register, since the bells/celeste that would be the
  // obvious choice are transients.
  debra_wilson: [
    "music/beds/debra_wilson/bed-01.mp3",
    "music/beds/debra_wilson/bed-02.mp3",
  ],
  // Two guitars trading a walking pattern: companionable rather than jaunty.
...
/**
 * Set used when a job's `channelKey` has no set of its own — the most neutral warm-acoustic of
 * the ten.
 */
