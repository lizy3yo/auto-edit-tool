...
} from "./server/longformVideo.ts";
const dir =
  "C:/Users/User/AppData/Local/Temp/claude/C--00-projects-auto-edit/a3776221-9052-47de-868a-b311319a9e05/scratchpad";
...
for (const [n, s] of cta.spans.entries()) {
  ...
  for (let i = 0; i < b.length; i += 20)
    if (nb(b.slice(i, i + 20).join(" "))) {
      f = i;
      break;
    }
  console.log(
    `CTA#${n + 1}: ${b.length}w at ${((s.start / all.length) * 100).toFixed(0)}%, cover ${f < 0 ? "NEVER FIRES" : "fires @" + f}`
  );
}
